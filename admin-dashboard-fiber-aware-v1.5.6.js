/*
 NatFiber Encyclopedia — Admin Dashboard Fiber-Aware v1.5.6
...
function setEditorVersion(){
  ...
  if(v && v.textContent!=='Editor v1.5.6') v.textContent='Editor v1.5.6';
}

function addStyle(){
  if($('#nfAdminDashV156Style'))return;
  const s=document.createElement('style');s.id='nfAdminDashV156Style';
  s.textContent=`
...
async function refresh(){
  ...
  try{
    ...
    let scope=$('#nfDashboardScopeV156');
    $('#nfDashboardScopeV14')?.remove();
    if(!scope){
      scope=document.createElement('div');
      scope.id='nfDashboardScopeV156';
