...
function setLegacyVisibility(host, show, keepIds=[]){
  if(!host) return;
  const keep=new Set(Array.isArray(keepIds)?keepIds:[keepIds]);
  [...host.children].forEach(el=>{
    if(el.classList.contains('nf-fiber-context') || keep.has(el.id)){
      el.classList.remove('nf-hidden-by-fiber');
      return;
    }
...
function enforceVisibility(){
  ...
  if(preview){
    // Product Media Release is fiber-aware and must remain visible for BOTH Ijuk and Sisal.
    // Previously it was treated as a legacy panel and hidden every 600 ms for NF-0001.
    setLegacyVisibility(preview,!isIjuk,[
      'ijukEnrichmentPanel',
      'nfProductReleasePanelV12'
    ]);
    $('#ijukEnrichmentPanel')?.classList.toggle('nf-hidden-by-fiber',!isIjuk);
    $('#nfProductReleasePanelV12')?.classList.remove('nf-hidden-by-fiber');
  }
}
