...
const PUBLIC_RENDERER_READY=false; // Safety gate: current public Global UI is still Sisal-shaped.
...
function addStyle(){
  ...
  s.textContent=`
    ...
    .nf-global-lock{margin:14px 0;padding:12px 14px;border:1px solid #e4c990;background:#fff8e8;border-radius:12px;color:#6b5421;font-size:11px;line-height:1.55}
  ...
  stage.innerHTML=`
    <div class="nf-global-stage-head">
      ...
      <div class="release-actions">
        ...
        <button id="publishIjukGlobalBtn" class="btn danger compact" disabled>${L('Publish terkunci','Publish locked')}</button>
    ...
    <div class="nf-global-lock"><strong>${L('Safety gate aktif:','Safety gate active:')}</strong> ${L('Tahap 2 belum dapat dipublish dari panel ini sampai renderer Global publik dibuat fiber-aware. Renderer publik saat ini masih memakai asumsi Sisal (seri “Annual commercial fibre production”, satuan kt, Top-5 producer).','Stage 2 cannot be published from this panel until the public Global renderer is fiber-aware. The current public renderer still assumes the Sisal data model (Annual commercial fibre production, kt units, Top-5 producers).')}</div>
...
function refCard(i){
  const r=i.record||{}; const u=r.url&&/^https?:/i.test(r.url)?r.url:''; return `<div class="nf-global-source-card"><strong>${esc(r.reference_id||i.record_id)} · ${esc(r.title||'')}</strong><small>${esc(r.publisher||r.authors||'')} · ${esc(r.year||'')}</small><small>${esc(r.verification_status||'')}</small>${u?`<a href="${esc(u)}" target="_blank" rel="noopener">${L('Buka sumber','Open source')} ↗</a>`:''}</div>`;
}
...
function render(p){
  ...
  $('#ijukGlobalSummary').innerHTML=[sumCard(s.selected_items??0,L('Selected','Selected')),sumCard(s.approved_items??0,L('Approved','Approved')),sumCard(s.hold_items??0,L('Hold','Hold')),sumCard(direct.length,L('Fibre-output records','Fibre-output records')),sumCard(dist.length,L('Distribution','Distribution')),sumCard(trade.length,L('Numeric trade','Numeric trade'))].join('');
  $('#ijukGlobalStatus').innerHTML=pub?`<strong>IJUK_GLOBAL_V1 published ✓</strong>`:`<strong>IJUK_GLOBAL_V1</strong> · ${L('PRIVATE candidate set. Belum ada data Tahap 2 yang dibuka ke publik.','PRIVATE candidate set. No Stage 2 data are public yet.')}`;
  $('#approveIjukGlobalBtn').disabled=role!=='ADMIN'||pub||allApproved;
  $('#publishIjukGlobalBtn').disabled=true;
  $('#publishIjukGlobalBtn').title=PUBLIC_RENDERER_READY?'':L('Renderer publik Ijuk Global belum siap.','Public Ijuk Global renderer is not ready.');
  const tradeHtml=trade.length?`<div class="nf-ijuk-grid">${trade.map(productionCard).join('')}</div>`:`<div class="nf-global-caveat"><strong>${L('Tidak ada angka perdagangan yang diadmit.','No numeric trade values admitted.')}</strong> ${L('HS 530500 menggabungkan coconut, abaca, ramie, dan other vegetable textile fibres; angka tersebut tidak boleh diklaim sebagai Ijuk/Arenga pinnata.','HS 530500 aggregates coconut, abaca, ramie, and other vegetable textile fibres; those totals cannot be claimed as Ijuk/Arenga pinnata trade.')}</div>`;
...
async function loadPreview(){
  ...
  try{
    ...
    if(!p) throw new Error('Editor session required.'); render(p);
  }catch(e){ ensureStage(); if($('#ijukGlobalBody')) $('#ijukGlobalBody').innerHTML=`<div class="message error">${esc(e.message||e)}</div>`; }
...
async function publish(){
  if(!PUBLIC_RENDERER_READY){ alert(L('Publish dikunci: renderer Global publik Ijuk belum fiber-aware.','Publish is locked: the public Ijuk Global renderer is not yet fiber-aware.')); return; }
}
