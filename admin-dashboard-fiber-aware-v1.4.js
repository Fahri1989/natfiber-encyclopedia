/*
 NatFiber Encyclopedia — Admin Dashboard Fiber-Aware v1.4
 Keeps database-wide context visible, but summary cards follow ACTIVE FIBER.
*/
...
function addStyle(){
  if($('#nfAdminDashV14Style'))return;
  const s=document.createElement('style');s.id='nfAdminDashV14Style';
  s.textContent=`
...
function setEditorVersion(){
  ...
  if(v) v.textContent='Editor v1.4';
}
...
async function refresh(){
  ...
  try{
    ...
    let scope=$('#nfDashboardScopeV14');
    if(!scope){scope=document.createElement('div');scope.id='nfDashboardScopeV14';scope.className='nf-dash-scope';$('#summaryCards').before(scope);}
    ...
    console.warn('[NatFiber] fiber-aware dashboard refresh failed',e);
  }finally{busy=false;}
}
